 ...
 *
 *   NEGATIVE: a magic marker `xxyyzz-not-in-dom` is embedded in the URL path
 *   (so VO *could* announce it via chrome). The rendered component omits it.
 *   Assertion: the captured log does NOT contain the marker. If this test
 *   starts failing, the pid-filter / focus-first pattern regressed — chrome
 *   noise is leaking into the log.
 *
 *   POSITIVE: the same shape of marker IS rendered as visible DOM text
...
/**
 * The App component reads `window.location.pathname` at render time, so a
 * direct history.pushState before render is sufficient to pick the right
 * route fixture.
 */
...
describe('DOM content vs Chrome URL bar (CONTEXT.md most-important)', () => {
  ...
  it.skipIf(!runVoTest)(
    ...
    async () => {
      ...
      // deliberately does NOT render it. If VO "reads" the URL bar we'd
      // pick it up via chrome-noise paths we're trying to filter out.
      const magicInUrl = 'xxyyzz-not-in-dom';
      navigate(`/test-page-${magicInUrl}`);
      await render(<App />); // branches on window.location.pathname

      // Anchor VO cursor in the page content, not the window chrome: click
      // the "Click me" button. Clicking a focused element via Playwright
      // sets focus inside the test viewport.
  ...
  it.skipIf(!runVoTest)(
    ...
    async () => {
      ...
      // Focus the button adjacent to the marker; VO announces DOM content
      // as it navigates through the page. Clicking the button anchors focus
      // in page content AND triggers VO to walk nearby siblings when
      // configured to read in sequence.
