...
**Domain:** Screen-reader automation for CI (macOS / VoiceOver v1, Linux / NVDA later). Native code core with TypeScript surface.
**Researched:** 2026-04-17
**Confidence:** HIGH on the core stack (Zig + napi-zig + AppleScript-VO + tart). MEDIUM on the GH-Actions-integration details for tart (correct at a high level; concrete YAML differs by runner provider). LOW on "napi-zig outperforms N-API + Rust" as a quantitative claim — see §Alternatives.
...
1. **"Yuku toolchain" is a misnomer in PROJECT.md.** Yuku is a JavaScript/TypeScript parser written in Zig. The actual TS↔Zig bridge that PROJECT.md is describing is **napi-zig** (`yuku-toolchain/napi-zig`). Yuku is the reference consumer of napi-zig, not the binding tool itself. All TS-binding decisions below are about **napi-zig**; Yuku is irrelevant to Shoki except as a model of "Zig library shipped via napi-zig to npm." This needs to be corrected in PROJECT.md.
2. **Target Zig 0.16.0 (stable).** Released 2026-04-13. napi-zig requires 0.16.0+ (its `build.zig.zon` pins `minimum_zig_version = "0.16.0-dev.2535+b5bd49460"`). Do not target 0.13/0.14 — they are too old for napi-zig and for the current `std.Build` shape used by the modern Zig build system.
3. **VoiceOver capture = AppleScript over `osascript`, period, for v1.** `tell application "VoiceOver" to return content of last phrase` is the canonical, documented, SIP-compatible path. Everything else (AXObserver notifications, private `AXSpeechSynthesizer`, audio scraping) is either out of reach without entitlements/private frameworks, or worse DX. Guidepup uses AppleScript; so does AccessLint's VoiceOver.js; so will Shoki. The improvement area vs Guidepup is **poll/buffer strategy and error handling**, not the underlying transport.
4. **tart is the right macOS CI VM.** Native Apple Virtualization.framework, OCI-registry distribution, and it's what Cirrus Runners is built on. The "pre-authorize VoiceOver + Accessibility permissions inside the VM image" workflow is a Packer/Ansible build-time concern — you bake it into a custom tart image and push to your own GHCR.
5. **Shoki does not need its own CI runner product.** The choice for users is: (a) GH-hosted `macos-latest` runners (expensive, no pre-configured VoiceOver — Shoki must do first-run setup), (b) self-hosted tart on their own Mac mini, (c) Cirrus Runners / GetMac as drop-in macOS runner services. Shoki's job is to ship a binary + TS SDK + a `shoki doctor` / `shoki setup` CLI, and a reference tart image. Not to sell runners.
6. **Distribution via optional-dependencies pattern.** napi-zig generates exactly this (`@scope/binding-<platform>-<arch>`) out of the box via `napi build --release`. Same pattern as esbuild, swc, biome, rolldown. No custom postinstall scripts. No downloading binaries at install time. Zero ambiguity here.
...
brew install --cask tart                 # for VM-backed local dev / image bakery
brew install packer ansible              # for building custom tart images
curl -fsSL https://ziglang.org/download/0.16.0/zig-aarch64-macos-0.16.0.tar.xz | tar -xJ   # or mlugg/setup-zig equivalent locally

# Inside the Shoki repo
zig fetch --save git+https://github.com/yuku-toolchain/napi-zig.git/#HEAD
npm install                              # installs napi-zig CLI + typescript + vitest + biome

# Build for current platform
npx napi-zig build

# Cross-compile everything + generate npm tree
npx napi-zig build --release
```
...
**Short version: not required for npm-only distribution in v1.**
...
From napi-zig's README, the publish flow is:

1. Define `addLib(b, napi_dep, .{ .name, .root, .target, .optimize, .npm = .{ .scope, .repository, .platforms } })` in `build.zig`.
2. `napi build --release` cross-compiles all platforms in `.npm.platforms` (default: Linux x64/arm64 gnu+musl, Linux arm, macOS x64/arm64, Windows x64/arm64, FreeBSD x64) and emits `npm/<name>/` with the main package + one sub-package per platform under `@<scope>/binding-<platform>-<arch>/`.
3. `napi npm-init --repo org/shoki --workflow publish.yml` publishes the first time and configures npm trusted publishing (OIDC). **No NPM_TOKEN** in GitHub Actions after that — `id-token: write` permission is enough.
4. `napi bump [patch|minor|major]` rewrites every package.json, creates an annotated git tag, pushes.
5. GitHub Actions picks up the tag, runs `npx napi-zig build --release` + `npx napi-zig publish`, publishes all sub-packages + main.

**This is the same pattern esbuild / swc / @node-rs/* / rolldown use.** It's the 2026 standard for shipping a native Node addon.
...
- **Guidepup VoiceOver source** — https://github.com/guidepup/guidepup/tree/main/src/macOS/VoiceOver — confirms AppleScript-over-osascript mechanism — **HIGH**
  - `lastSpokenPhrase.ts`: `tell application "VoiceOver" ... return content of last phrase`
  - `itemText.ts`: `return text under cursor of vo cursor`
  - `runAppleScript.ts`: `execFile("/usr/bin/osascript", ..., stdin=<script>)`
  - `LogStore.ts`: the polling + stabilization strategy we should learn from (and improve)
...
---
*Stack research for: macOS VoiceOver CI automation with a Zig native core + TypeScript SDK*
*Researched: 2026-04-17*
