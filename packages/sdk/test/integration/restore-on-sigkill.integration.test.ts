 ...
 * `ogmios restore-vo-settings` CLI reads that file and re-applies every key.
 *
 * Sequence:
 *   1. Snapshot all 9 VO plist keys (reference).
 *   2. Fork sigkill-child.ts with $OGMIOS_SNAPSHOT_PATH pointing at a
 *      tempfile. Wait for "started" IPC — means voiceOver.start() finished
 *      AND the snapshot file was written.
 *   3. Assert the snapshot file exists and has _ogmios_snapshot_version.
 *   4. SIGKILL the child; wait for exit.
 *   5. Assert at least one plist key currently differs from the pre-test
 *      snapshot (proves ogmios actually altered settings before the kill).
 *   6. Run `ogmios restore-vo-settings --path <tempfile>`.
...
maybeDescribe('restore-on-sigkill (Plan 07-05 CONTEXT.md § Settings restore)', () => {
  it(
    'SIGKILL crash → ogmios restore-vo-settings restores all 9 plist keys',
    async () => {
      // 1. Reference snapshot of all 9 keys before we do anything.
      const before = await readSnapshot();
      expect(Object.keys(before)).toHaveLength(9);

      // Tempfile for the snapshot — keeps ~/.ogmios/vo-snapshot.plist untouched.
      const tmpRoot = mkdtempSync(join(tmpdir(), 'ogmios-sigkill-'));
      ...
      const child = spawn(process.execPath, ['--import', 'tsx/esm', childPath], {
        ...
        env: { ...process.env, OGMIOS_SNAPSHOT_PATH: snapshotPath },
      });
      ...
      try {
        ...
        expect(xml).toContain('_ogmios_snapshot_version');
        ...
        // ogmios actually wrote its defaults before the kill. (Some keys may
        // legitimately match if the user already had ogmios's default value.)
        const mid = await readSnapshot();
        const anyChanged = Object.keys(before).some((k) => before[k] !== mid[k]);
        expect(
          anyChanged,
          'no plist key differed from pre-test — ogmios never wrote its defaults?',
        ).toBe(true);
        ...
        expect(
          after,
          'plist not restored to pre-test state after `ogmios restore-vo-settings`',
        ).toEqual(before);
