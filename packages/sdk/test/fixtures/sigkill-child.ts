 ...
 * then invokes `ogmios restore-vo-settings --path <fixture-tempfile>` and
 * asserts every plist key returned to its pre-test value.
 *
 * The child honors $OGMIOS_SNAPSHOT_PATH so the parent can point it at a
 * tempfile and avoid polluting the user's ~/.ogmios/vo-snapshot.plist.
 */
