...
describe('runSetup', () => {
  ...
  beforeEach(async () => {
    installDir = await mkdtemp(join(tmpdir(), 'ogmios-setup-cmd-'));
  });
  ...
  it('--dry-run prints nothing to fs or network; returns noop with URL context', async () => {
    ...
    expect(result.launched).toBe(false);
    expect(result.downloadedFromUrl).toContain('ogmios-darwin');
    expect(fetchFn).not.toHaveBeenCalled();
    expect(exec).not.toHaveBeenCalled();
  });

  it('--install-dir <path> routes everything into the custom dir', async () => {
    const altDir = await mkdtemp(join(tmpdir(), 'ogmios-altdir-'));
    try {
      const result = await runSetup({
        installDir: altDir,
        fetch: await mkReleaseFetch(),
