...
const TAMPERED_ZIP = join(FIXTURE_DIR, 'shoki-darwin-arm64.tampered.zip');
...
describe('downloadAndVerifyZip', () => {
  ...
  beforeEach(async () => {
    workDir = await mkdtemp(join(tmpdir(), 'shoki-setup-download-test-'));
  });
  ...
  it('downloads + verifies a well-formed zip whose SHA matches the sidecar', async () => {
    ...
    const zipUrl =
      'https://github.com/shoki/shoki/releases/download/app-v0.1.0/shoki-darwin-arm64.zip';
  ...
  it('rejects with URL + 404 context when the zip fetch returns 404', async () => {
    ...
    ).rejects.toThrowError(/(404|not found).*shoki-darwin-arm64\.zip/is);
  ...
  it('supports baseUrl override for test fixture servers', async () => {
    ...
    const zipUrl = `${baseUrl}/app-v0.1.0/shoki-darwin-arm64.zip`;
    const shaUrl = `${zipUrl}.sha256`;
