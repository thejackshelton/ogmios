/**
 * Unit tests for the `ogmios restore-vo-settings` CLI logic — Plan 07-05.
 *
 ...
 * snapshot file includes `_ogmios_snapshot_{version,pid,ts_unix}` magic keys
...
const tmpRoot = mkdtempSync(join(tmpdir(), 'ogmios-restore-test-'));
...
<plist version="1.0">
<dict>
    <key>_ogmios_snapshot_domain</key>
    <string>${domain}</string>
${keys}${extrasXml}    <key>_ogmios_snapshot_version</key>
    <integer>${version}</integer>
    <key>_ogmios_snapshot_pid</key>
    <integer>12345</integer>
    <key>_ogmios_snapshot_ts_unix</key>
    <integer>${ts}</integer>
</dict>
</plist>
`;
...
describe('restoreVoSettingsFromSnapshot', () => {
  ...
  it('rejects with SNAPSHOT_UNRECOGNIZED when the magic key is absent', async () => {
    // Plist without the _ogmios_snapshot_version marker.
