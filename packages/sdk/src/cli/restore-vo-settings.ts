 ...
 *  - `SNAPSHOT_UNRECOGNIZED` — magic `_shoki_snapshot_version` absent (2)
...
/** Shoki's 9 canonical VO plist keys + their `defaults` type flag.
 ...
 */
const SHOKI_KEYS: ReadonlyArray<{
  key: string;
...
/**
 * Extract a key's value from the snapshot plist XML. Shoki-written snapshots
 * use a narrow set of tag shapes — we don't need a full plist parser.
 * Returns `null` when the key is absent or matches the `__SHOKI_MISSING__`
...
function extractValue(
  ...
  const m = xml.match(re);
  if (!m) return null;
  if (m[1] === '__SHOKI_MISSING__') return { kind: 'missing' };
  return { kind: 'present', value: m[1]! };
}

function extractDomain(xml: string): string {
  const m = xml.match(/<key>_shoki_snapshot_domain<\/key>\s*<string>([^<]+)<\/string>/);
  return m?.[1] ?? 'com.apple.VoiceOver4/default';
}

/**
 * Re-apply every key in a shoki snapshot file to the host's plist via
...
export async function restoreVoSettingsFromSnapshot(
  ...
  // Magic version check — reject files that don't look like shoki snapshots.
  const versionMatch = xml.match(
    /<key>_shoki_snapshot_version<\/key>\s*<integer>(\d+)<\/integer>/,
  );
  ...
  const tsMatch = xml.match(
    /<key>_shoki_snapshot_ts_unix<\/key>\s*<integer>(\d+)<\/integer>/,
  );
  ...
  for (const { key, type } of SHOKI_KEYS) {
