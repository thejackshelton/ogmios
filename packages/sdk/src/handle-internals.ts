import type { AwaitStableLogOptions, ShokiEvent } from './screen-reader.js';
 ...
 * event, (b) callbacks receive a typed ShokiEvent directly instead of needing
 * to unwrap `CustomEvent.detail`, and (c) it keeps the hot path allocation-free.
 */
export class LogStore {
  private events: ShokiEvent[] = [];
  private subscribers: Set<(event: ShokiEvent) => void> = new Set();
