...
export interface ShokiVitestPluginOptions {
  ...
  /** Scan test files for @shoki/core/vitest/browser imports during the config hook. Default true. */
  detectVoiceOverImports?: boolean;
}

const IMPORT_NEEDLE = '@shoki/core/vitest/browser';
const TEST_EXTS = new Set(['.ts', '.tsx', '.js', '.jsx', '.mts', '.mjs']);

/**
 * Walk a project tree looking for any JS/TS file that imports
 * `@shoki/core/vitest/browser`. Skips node_modules, .git, dist, and dotfiles.
    ...
    config: async (cfg) => {
      ...
      if (options.autoSingleThread && options.detectVoiceOverImports) {
        ...
        if (found) {
          ...
          if (existing === false) {
            console.warn(
              "[@shoki/core/vitest] Detected `import ... from '@shoki/core/vitest/browser'` in test files, " +
