...
export function ogmiosVitest(opts: OgmiosVitestPluginOptions = {}): Plugin {
  const options = {
    ...
    config: async (cfg) => {
      ...
      if (options.autoSingleThread && options.detectVoiceOverImports) {
        ...
        if (found) {
          ...
          if (existing === false) {
            console.warn(
              "[ogmios/vitest] Detected `import ... from 'ogmios/vitest/browser'` in test files, " +
                'but poolOptions.threads.singleThread is explicitly `false`. VoiceOver is a system singleton; ' +
                'cross-test interference is likely. Remove the override or set `ogmiosVitest({ autoSingleThread: false })`.',
            );
