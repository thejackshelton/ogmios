#!/usr/bin/env bash
# Package ShokiRunner.app + ShokiSetup.app into a single release zip + SHA256.
#
...
#
# Phase 10 Plan 03: this is the producer side of the `shoki setup` download flow
# (Plan 10-02 parses the SHA256 file with that exact "<64-hex>  <basename>\n"
# format). The renamed bundles match the user-visible names from 10-CONTEXT.md
# § "Distribution: unified Shoki.app.zip per platform": Shoki.app (formerly
# ShokiRunner.app) and "Shoki Setup.app" (formerly ShokiSetup.app). Bundle
# identifiers (org.shoki.runner / org.shoki.setup) are unaffected so TCC
# grants persist.
#
...
RUNNER_SRC="$HELPER_DIR/.build/ShokiRunner.app"
SETUP_SRC="$HELPER_DIR/.build/ShokiSetup.app"
...
ZIP_NAME="shoki-darwin-${ARCH}.zip"
SHA_NAME="${ZIP_NAME}.sha256"
ZIP_PATH="$OUT_DIR/$ZIP_NAME"
SHA_PATH="$OUT_DIR/$SHA_NAME"

# Stage renamed bundles in a temp dir; never mutate the build outputs so a
# subsequent `package-app-zip.sh --arch <other>` run (or a re-run of the same
# arch) sees the canonical ShokiRunner.app / ShokiSetup.app names.
STAGE="$(mktemp -d)"
trap 'rm -rf "$STAGE"' EXIT

echo "[package-app-zip] Staging renamed bundles in $STAGE"
ditto "$RUNNER_SRC" "$STAGE/Shoki.app"
ditto "$SETUP_SRC"  "$STAGE/Shoki Setup.app"

echo "[package-app-zip] Creating $ZIP_NAME"
# ditto -c -k accepts exactly one source. Archiving the STAGE dir (without
# --keepParent) places its immediate children — Shoki.app/ and
# Shoki Setup.app/ — at the zip root, which is what Plan 10-02's extractor
# relies on (see packages/sdk/src/cli/setup-install.ts: it looks for
# <installDir>/Shoki.app and <installDir>/Shoki Setup.app after extraction).
#
