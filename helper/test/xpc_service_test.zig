...
//   1. ping returns "ogmios-runner-pong"
...
//   6. mach_service_name constant equals "org.ogmios.runner"

const std = @import("std");
const testing = std.testing;
const svc = @import("../src/runner/xpc_service.zig");

test "ping handler returns ogmios-runner-pong" {
    ...
    try testing.expectEqualStrings("ogmios-runner-pong", reply);
}
...
test "mach_service_name is the frozen wire constant org.ogmios.runner" {
    const c_name: [*:0]const u8 = svc.mach_service_name;
    const name_slice = std.mem.span(c_name);
    try testing.expectEqualStrings("org.ogmios.runner", name_slice);
}
