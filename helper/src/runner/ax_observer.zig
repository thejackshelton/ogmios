// Zig port of `helper/Sources/ShokiRunnerService/AXObserver.swift` (Phase 08
// Plan 02 Task 1).
//
...
// (`EventCallback`) which dispatched back into `ShokiRunnerService`.
//
...
/// by `ShokiRunnerService.swift` so a client can map them 1:1 on both sides
/// of the XPC boundary.
pub const AxError = error{
    /// Caller passed `target_pid <= 0`. Matches ShokiRunnerService.swift:56.
    InvalidPid,
