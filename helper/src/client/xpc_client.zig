// Zig port of `helper/Sources/OgmiosXPCClient/OgmiosXPCClient.swift` — the
...
//
//     ogmios_xpc_connect()                           -> ?*anyopaque
//     ogmios_xpc_set_event_callback(h, cb, userdata) -> void
//     ogmios_xpc_start_ax_observer(h, pid)           -> i32
//     ogmios_xpc_stop_ax_observer(h)                 -> i32
//     ogmios_xpc_disconnect(h)                       -> void
//
...
//
// The handle returned by `ogmios_xpc_connect` is opaque to callers — we use it
...
//
//   * `ogmios_xpc_connect`      → creates a Session, opens a Mach-service
//                                 connection to org.ogmios.runner, returns
//                                 the Session pointer.
//   * `ogmios_xpc_set_event_callback` → stores the C function pointer.
//   * `ogmios_xpc_start_ax_observer`  → sends a `startAXObserver` XPC message.
//   * `ogmios_xpc_stop_ax_observer`   → sends a `stopAXObserver` XPC message.
//   * `ogmios_xpc_disconnect`         → cancels the connection and frees the
//                                       Session.
...
/// Per-connection state. Leaked on `ogmios_xpc_disconnect` via std.heap.c_allocator.
...
///
/// Production semantics MATCH Swift's `ogmios_xpc_connect`:
///  - creates NSXPCConnection against `org.ogmios.runner`
///  - exports this side as the "client" (receives receiveAXEvent callbacks)
///  - returns a retained handle
export fn ogmios_xpc_connect() ?XpcHandle {
...
export fn ogmios_xpc_set_event_callback(
    h: XpcHandle,
    cb: EventCCallback,
    userdata: ?*anyopaque,
) void {
...
/// was healthy.
export fn ogmios_xpc_start_ax_observer(h: XpcHandle, pid: i32) i32 {
    const session: *Session = @ptrCast(@alignCast(h));
...
export fn ogmios_xpc_stop_ax_observer(h: XpcHandle) i32 {
    const session: *Session = @ptrCast(@alignCast(h));
    const conn = session.connection orelse return -1;
...
/// `h` is invalid.
export fn ogmios_xpc_disconnect(h: XpcHandle) void {
    const session: *Session = @ptrCast(@alignCast(h));
