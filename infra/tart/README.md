# Ogmios VO-ready tart images

Reference pre-baked macOS images for ogmios CI. Published as:

- `ghcr.io/thejackshelton/ogmios/macos-vo-ready:sonoma` — macOS 14, slim (<15 GB)
- `ghcr.io/thejackshelton/ogmios/macos-vo-ready:sequoia` — macOS 15, slim
- `ghcr.io/thejackshelton/ogmios/macos-vo-ready:tahoe` — macOS 26, slim (when base image ships)
- `ghcr.io/thejackshelton/ogmios/macos-vo-ready:<ver>@full` — same + Xcode + iOS simulators (~50 GB); opt-in

Each image includes:

- VoiceOver AppleScript enabled (`SCREnableAppleScript=1`)
- Accessibility + Automation TCC grants pre-applied to `org.ogmios.runner`
...
Ogmios tests need a macOS environment where VoiceOver can be driven from
AppleScript AND the process driving it (the ogmios helper bundle) has the
right TCC grants AND no background apps are emitting announcements that
leak into captures.

On a fresh macOS CI runner none of these things are true. `ogmios doctor`
...
# ~45 min. Produces a local tart VM named `ogmios-vo-ready-sonoma`.
```

Build the `@full` variant (Xcode + simulators preserved):

```bash
packer build -var 'full_image=true' -var 'output_name=ogmios-vo-ready-sonoma-full' packer/sonoma.pkr.hcl
```
...
# In another terminal:
ssh admin@$(tart ip ogmios-vo-ready-sonoma)
# Password: admin
# Run smoke test:
...
ogmios use cases — driving VoiceOver in a web app via Playwright +
Vitest.

**`@full`:** retains Xcode 16+, iOS simulators, additional dev tools.
~50 GB. For users who want ogmios to eventually drive iOS VoiceOver via
XCUITest (deferred to v2+; see `REQUIREMENTS.md` SR2-01 / PLAT-03). The
image is published but not exercised by the ogmios test matrix today.

Opt in via image tag: `ghcr.io/thejackshelton/ogmios/macos-vo-ready:sonoma@full`.
...
1. **TCC schema drift.** The TCC.db schema has changed across macOS
   ...
   the grant row non-functional. Symptom: `ogmios doctor` reports
...
ssh -o StrictHostKeyChecking=no admin@$IP 'ogmios doctor --json --quiet'
```
