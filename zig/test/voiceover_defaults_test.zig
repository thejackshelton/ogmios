...
test "keyCatalog entry 0 is SCREnableAppleScript bool=true" {
    ...
    try std.testing.expectEqual(true, catalog[0].shoki_default.boolean);
}
...
test "keyCatalog entry 7 is SpeakChannel = voice.Alex" {
    ...
    try std.testing.expectEqualStrings("com.apple.speech.synthesis.voice.Alex", catalog[7].shoki_default.string);
}
