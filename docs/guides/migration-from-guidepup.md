...
If you're here, you've used [Guidepup](https://github.com/guidepup/guidepup) — the pioneering OSS library for VoiceOver automation — and want to know when and why to switch to Ogmios.

**Short answer:** Ogmios is observe-only by design. If you need to _drive_ VoiceOver's rotor, navigate headings, or interact with the VO cursor, stay on Guidepup. If you want to **capture** what VoiceOver says while your existing framework (Vitest, Playwright, XCUITest) drives the app, Ogmios is a better fit — with first-class CI support, structured events instead of `string[]`, and a stable TCC trust anchor that doesn't re-prompt every Node upgrade.

## Positioning

| Concern | Guidepup | Ogmios |
|---------|----------|--------|
| **API surface** | VO driver (keyPress, rotor nav, interact) + capture | Capture only |
| **Data shape** | `string[]` of phrases | `OgmiosEvent[]` — `{ phrase, ts, source, role?, name?, interrupt? }` |
...
Ogmios's thesis: **observing is 90% of what accessibility tests need, and making that bit excellent wins more than trying to match Guidepup feature-for-feature.** For the remaining 10% (driving VO), your framework's keyboard API dispatches Ogmios's exported 226-gesture catalog — see [CAP-16](https://github.com/thejackshelton/ogmios/blob/main/.planning/REQUIREMENTS.md).
...
- **`start({ mute: true })`** — Ogmios mutes VO by default in tests; Guidepup inherits your system settings.
...
Nothing stops you. Guidepup's navigation helpers can drive VO while Ogmios captures — just make sure only one library boots VO.

## Limits to be honest about

- **Ogmios is macOS-only in v1.** Guidepup ships NVDA support already. If you need NVDA today, Guidepup is your only choice. Ogmios's v1.1 roadmap adds NVDA to validate the driver abstraction (see [EXT-01](/background/architecture#driver-extensibility)).
- **Ogmios doesn't drive VO.** If your test asserts on post-`next()` rotor state, Guidepup is the right tool.

## Want to migrate?

Open an issue on the [ogmios repo](https://github.com/thejackshelton/ogmios) with the specific Guidepup APIs your suite uses and we'll map them to ogmios equivalents (or explicitly file them as out of scope). The [API reference](/api/sdk) is the source of truth for the full surface.
