...
- **`shoki/vitest/setup`** — `expect.extend(...)` setup file wiring Shoki's matchers (from `shoki/matchers`) into Vitest's `expect`.
...
All four Shoki matchers (at `shoki/matchers`, wired through `shoki/vitest/setup`) accept `WireShokiEvent[]` transparently, so test code stays uniform across browser and Node contexts.
...
- First `shokiStart` boots VO via `shoki`.
...
- [`shoki`](/api/sdk) — the Node-side API this wraps.
- [Matchers API](/api/matchers) — `shoki/matchers` (pure fns) + `shoki/vitest/setup` (wiring).
