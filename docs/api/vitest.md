...
- **`@shoki/core/vitest/browser`** — browser-safe API (`voiceOver.start()`, `session.awaitStable()`).
- **`@shoki/core/vitest/setup`** — `expect.extend(...)` setup file wiring Shoki's matchers (from `@shoki/core/matchers`) into Vitest's `expect`.
...
interface ShokiVitestOptions {
   ...
   * enabled when any browser-side file imports `@shoki/core/vitest/browser`.
   */
...
1. Registers 10 `BrowserCommand`s on the Vitest server:
   ...
   - `shokiAwaitStable`
   - `shokiDropped`
2. Scans test files for imports of `@shoki/core/vitest/browser`. If found, sets `poolOptions.threads.singleThread = true` — VoiceOver is a system singleton.
...
All four Shoki matchers (at `@shoki/core/matchers`, wired through `@shoki/core/vitest/setup`) accept `WireShokiEvent[]` transparently, so test code stays uniform across browser and Node contexts.
...
- First `shokiStart` boots VO via `@shoki/core`.
...
- [`@shoki/core`](/api/sdk) — the Node-side API this wraps.
- [Matchers API](/api/matchers) — `@shoki/core/matchers` (pure fns) + `@shoki/core/vitest/setup` (wiring).
